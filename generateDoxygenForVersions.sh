...
while IFS= read -r version; do
  ...
  echo ""
  docker run --user ${UID} --rm -v `pwd`:/home dolibarr/doxygen:1.8.16 /bin/sh -c "( cat dolibarr.doxyfile ; echo 'PROJECT_NUMBER=$version' ; echo 'OUTPUT_DIRECTORY=build_$VERSIONSFILE' ; echo 'HTML_OUTPUT=$version' ) | doxygen -"

  echo "Generating Doxygen LaTeX doc for Dolibarr version $version"
  echo ""
  docker run --user ${UID} --rm -v `pwd`:/home dolibarr/doxygen:1.8.16 /bin/sh -c "( cat dolibarr.doxyfile ; echo 'PROJECT_NUMBER=$version' ; echo 'OUTPUT_DIRECTORY=build_$VERSIONSFILE' ; echo 'GENERATE_HTML=NO' ; echo 'GENERATE_LATEX=YES' ; echo 'LATEX_OUTPUT=pdf_$version' ) | doxygen -"
  ...
  docker run --rm -v `pwd`:/home dolibarr/doxygen:1.8.16 /bin/sh -c "cd build_$VERSIONSFILE/pdf_$version/ && make && chmod a+rw refman*"
  cd ${DIRPATH}
